...
const BoxProduct: React.FC<BoxProductProps> = memo(({ item }) => {
  ...
  const getImageSrc = (index: number = 0) => {
    if (item.imageUrls && item.imageUrls.length > index) {
      ...
      return `${import.meta.env.VITE_BASE_URL || 'http://localhost:5000'}/${imageUrl.replace(/\\/g, "/")}`;
    }
    return "/default.png";
  };
  ...
  return isLoaded ? (
    <div className={`group cursor-pointer h-full transition-all duration-700 ease-out transform ${isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`}>
      <Card
        hoverable
        className="h-full flex flex-col overflow-hidden border-0 shadow-sm hover:shadow-lg transition-all duration-300 bg-white"
        cover={
      ...
      >
        ...
        <div className="flex-1 flex flex-col justify-between space-y-3">
          ...
          <div
            ...
            onClick={() => navigate(`/product/${item._id}`)}
          >
            <h3 className="text-sm font-medium text-gray-900 hover:text-amber-700 transition-colors duration-200 !mb-0"
              style={{
                display: '-webkit-box',
                WebkitLineClamp: 2,
    ...
    <div className="group cursor-pointer h-full animate-pulse">
      <Card className="h-full flex flex-col overflow-hidden border-0 shadow-sm bg-white" styles={{ body: { padding: '16px', flex: 1, display: 'flex', flexDirection: 'column' } }}>
