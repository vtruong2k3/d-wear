...
import { PlusOutlined, UploadOutlined, EyeOutlined, DeleteOutlined, ThunderboltOutlined } from "@ant-design/icons";
...
const ProductEdit = () => {
  ...
  useEffect(() => {
    const fetchSelectOptions = async () => {
      try {
        ...
        const brandsData = Array.isArray(brandRes.data.data) ? brandRes.data.data : [];
        const categoriesData = Array.isArray(categoryRes.data.data) ? categoryRes.data.data : [];
        const sizesData = Array.isArray(sizeRes.data.data) ? sizeRes.data.data : [];
        const colorsData = Array.isArray(colorRes.data.data) ? colorRes.data.data : [];
  ...
  const beforeUploadVariantImage = (file: File) => {
    const isValidType = file.type === "image/jpeg" || file.type === "image/png" || file.type === "image/webp" || file.type === "image/avif";
    if (!isValidType) {
      message.error("Chỉ cho phép ảnh JPEG, PNG hoặc WEBP!");
      return Upload.LIST_IGNORE;
    }
      ...
      try {
        ...
        setCurrentImages(
          (product.product.imageUrls || []).map((url: string, index: number) => {
            const fullUrl = url.startsWith("http") ? url : `http://localhost:5000/${url.replace(/\\/g, "/")}`;
            return { uid: `current-${index}`, name: `image-${index}`, url: fullUrl };
          })
        );
        ...
        setVariants(
          (product.variants || []).map((variant: IVariants, index: number) => ({
            ...
            image: (variant.image || []).map((url: string, i: number) => {
              ...
              const fullUrl = normalizedPath.startsWith("http") ? normalizedPath : `http://localhost:5000/${normalizedPath}`;
  ...
  const onFinish = async (values: IProductAdd) => {
    try {
      ...
      variants.forEach((variant, idx) => {
        ...
        if (variant.price === null || variant.price === undefined || variant.price <= 0) errs.push("price");
      ...
      if (!isValid) {
        ...
        message.error("Vui lòng nhập đầy đủ và hợp lệ thông tin cho các biến thể!");
        return;
      }
      ...
      currentImages.forEach((img) => formData.append("existingImageUrls", img.url));
      imageList.forEach((file) => {
        if (file.originFileObj) formData.append("productImage", file.originFileObj);
      });
      ...
      const plainVariants = variants.map((v) => {
        const oldImages = v.image
          .filter((img) => {
            ...
            return !file.originFileObj && (file.url || file.rawFileName || file.name);
          })
          .map((img) => {
            if (typeof img === "string") return img;
            const file = img as UploadFileWithRaw;
            return file.url || file.rawFileName || file.name;
          });
  ...
  const generateBulkVariants = () => {
    ...
    bulkColors.forEach((c) => {
      bulkSizes.forEach((s) => {
        ...
        if (!exists) {
          newVariants.push({ size: s, color: c, price: bulkPrice, stock: bulkStock, image: [] });
        }
      });
    });
  ...
  const addVariant = () => {
    setVariants([...variants, { size: "", color: "", price: undefined, stock: 0, image: [] }]);
  };

  const handleVariantChange = <K extends keyof VariantForm>(index: number, field: K, value: VariantForm[K]) => {
  ...
  const columns = [
    {
      ...
      render: (_: any, record: VariantForm, index: number) => (
        ...
        >
          {colors.map((item) => (
            <Option key={item._id} value={item.color_name}>{item.color_name}</Option>
          ))}
        </Select>
      ),
    },
    {
      ...
      render: (_: any, record: VariantForm, index: number) => (
        ...
        >
          {sizes.map((item) => (
            <Option key={item._id} value={item.size_name}>{item.size_name}</Option>
          ))}
        </Select>
      ),
    },
    ...
    },
    {
      ...
      render: (_: any, __: any, index: number) => (
        <Button danger type="text" icon={<DeleteOutlined />} onClick={() => removeVariant(index)} />
      ),
    },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <Form layout="vertical" form={form} onFinish={onFinish} requiredMark={false}>
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
            <div className="xl:col-span-2 flex flex-col gap-8">
              <Card bordered={false} className="shadow-sm rounded-xl" title={<span className="text-lg font-bold text-gray-800">Thông Tin Cơ Bản</span>}>
                <Form.Item label={<span className="font-semibold">Tên sản phẩm</span>} name="product_name" rules={[{ required: true, message: "Vui lòng nhập tên sản phẩm!" }]}>
                  <Input placeholder="Nhập tên sản phẩm..." size="large" className="rounded-lg" />
                </Form.Item>
                <Form.Item label={<span className="font-semibold">Mô tả chi tiết</span>} name="description" rules={[{ required: true, message: "Vui lòng nhập mô tả!" }]}>
                  <TextArea rows={6} placeholder="Nhập mô tả chi tiết về sản phẩm..." className="rounded-lg resize-none" />
                </Form.Item>
              </Card>
              ...
              <Card bordered={false} className="shadow-sm rounded-xl" title={<span className="text-lg font-bold text-gray-800">Hình Ảnh Sản Phẩm (Tối đa 8 ảnh)</span>}>
                      ...
                      {currentImages.map((image, index) => (
                        <div key={index} className="relative group aspect-square rounded-lg overflow-hidden border border-gray-200">
                          ...
                          <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-40 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100 gap-2">
                            <button type="button" onClick={() => handleCurrentImagePreview(image.url)} className="bg-white text-gray-700 hover:text-blue-600 p-2 rounded-full shadow-lg">
                              <EyeOutlined />
                            </button>
                            <button type="button" onClick={() => handleRemoveCurrentImage(index)} className="bg-white text-gray-700 hover:text-red-600 p-2 rounded-full shadow-lg">
                              <DeleteOutlined />
                            </button>
                          </div>
                        </div>
                ...
                <Form.Item name="productImage">
                  <div className="text-gray-800 font-semibold text-sm mb-2">Thêm ảnh mới</div>
                  <Upload
                    listType="picture-card"
                    fileList={imageList}
              ...
              <Card bordered={false} className="shadow-sm rounded-xl border-t-4 border-t-blue-500" title={<span className="text-lg font-bold text-blue-700 flex items-center"><ThunderboltOutlined className="mr-2" /> Sinh Biến Thể Nhanh</span>}>
                <div className="bg-blue-50 p-4 rounded-lg mb-4 text-sm text-blue-800">
                  Thêm mới dải màu/size hàng loạt vào danh sách biến thể hiện tại.
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-semibold mb-2">Chọn Màu sắc</label>
                    <Select mode="multiple" allowClear className="w-full" placeholder="VD: Đen, Trắng..." value={bulkColors} onChange={setBulkColors} size="large">
                  ...
                  <div>
                    ...
                    <Select mode="multiple" allowClear className="w-full" placeholder="VD: S, M, L..." value={bulkSizes} onChange={setBulkSizes} size="large">
                  ...
                  <div>
                    ...
                    <InputNumber className="w-full" size="large" min={0} value={bulkPrice} onChange={setBulkPrice} formatter={(value) => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ",")} parser={(value) => Number(value?.replace(/\$\s?|(,*)/g, "") || 0)} />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold mb-2">Tồn kho chung</label>
                    <InputNumber className="w-full" size="large" min={0} value={bulkStock} onChange={setBulkStock} />
                  </div>
                </div>
                <div className="mt-4 flex justify-end">
                  <Button type="primary" className="bg-blue-600" size="large" icon={<ThunderboltOutlined />} onClick={generateBulkVariants}>Tạo Hàng Loạt</Button>
                </div>
              </Card>
              ...
              <Card bordered={false} className="shadow-sm rounded-xl" title={
                <div className="flex justify-between items-center">
                  ...
                  <Button onClick={addVariant} icon={<PlusOutlined />} type="dashed">Thêm thủ công</Button>
                </div>
              }>
                {variants.length > 0 ? (
                  <Table 
                  ...
                  <div className="text-center py-10 text-gray-400">Chưa có biến thể nào. Hãy thêm mới.</div>
                )}
              </Card>
            </div>
            ...
            <div className="flex flex-col gap-8">
              <Card bordered={false} className="shadow-sm rounded-xl" title={<span className="text-lg font-bold text-gray-800">Mức Giá Cơ Bản</span>}>
                <Form.Item label={<span className="font-semibold">Giá bán (VNĐ)</span>} name="basePrice" rules={[{ required: true, message: "Vui lòng nhập giá!" }, { type: "number", min: 1000, message: "Số tiền phải lớn hơn 1000đ" }]}>
                  <InputNumber<number> min={0} className="w-full rounded-lg" size="large" formatter={(value) => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ",")} parser={(value) => Number(value?.replace(/\$\s?|(,*)/g, "") || 0)} placeholder="VD: 250,000" />
                </Form.Item>
              </Card>

              <Card bordered={false} className="shadow-sm rounded-xl" title={<span className="text-lg font-bold text-gray-800">Phân Loại</span>}>
                <Form.Item label={<span className="font-semibold">Thương hiệu (Brand)</span>} name="brand_id" rules={[{ required: true, message: "Vui lòng chọn brand!" }]}>
                  <Select placeholder="Chọn thương hiệu..." size="large" className="rounded-lg" showSearch optionFilterProp="children">
                    {brands.map((brand) => <Option key={brand._id} value={brand._id}>{brand.brand_name}</Option>)}
                  </Select>
                </Form.Item>
                <Form.Item label={<span className="font-semibold">Danh mục (Category)</span>} name="category_id" rules={[{ required: true, message: "Vui lòng chọn category!" }]}>
              ...
              <Card bordered={false} className="shadow-sm rounded-xl" title={<span className="text-lg font-bold text-gray-800">Thuộc Tính Bổ Sung</span>}>
                <Form.Item label={<span className="font-semibold">Giới tính</span>} name="gender" rules={[{ required: true, message: "Vui lòng chọn giới tính!" }]}>
                ...
                <Form.Item label={<span className="font-semibold">Chất liệu</span>} name="material" rules={[{ required: true, message: "Vui lòng nhập chất liệu!" }]}>
          ...
          <div className="fixed bottom-0 left-0 w-full bg-white shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] p-4 z-50 flex justify-end gap-4 px-8 xl:px-32">
            <Button size="large" className="min-w-[120px] rounded-lg font-medium" onClick={() => navigate("/admin/products")}>Hủy bỏ</Button>
            <Button type="primary" htmlType="submit" loading={loadingAction} size="large" className="min-w-[150px] bg-blue-600 rounded-lg font-semibold shadow-lg">
              {loadingAction ? "Đang xử lý..." : "Lưu Sản Phẩm"}
            </Button>
          </div>
        </Form>
      </div>
