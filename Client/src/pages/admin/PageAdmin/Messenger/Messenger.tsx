...
import type { WaitingChatRoom, ChatMessage, ChatRoomUser } from '../../../../types/message/IMessage';
// Đổi tên hàm import cho rõ nghĩa hoặc giữ nguyên getWaitingRooms đều được
import { getMessages, sendMessage, joinChatRoom } from '../../../../services/client/messageService';
...
const AdminMessageManager = () => {
    ...
    const [selectedCustomer, setSelectedCustomer] = useState<WaitingChatRoom | null>(null);
    ...
    useEffect(() => {
        ...
        const fetchInitialRooms = async () => {
            try {
                ...
                const sortedRooms = rooms.sort((a: WaitingChatRoom, b: WaitingChatRoom) => new Date(b.lastActive || 0).getTime() - new Date(a.lastActive || 0).getTime());
        ...
        const handleReceiveMessage = (newMessage: ChatMessage) => {
            ...
            if (senderId !== user._id) {
                setMessages(prev => ({ ...prev, [newMessage.chatRoomId]: [...(prev[newMessage.chatRoomId] || []), newMessage] }));
            }
        };
        ...
        const handleUpdateRoomList = (message: ChatMessage) => {
            setCustomers(prevCustomers => {
                const roomExists = prevCustomers.some(c => c._id === message.chatRoomId);
                if (roomExists) {
                    const updatedCustomers = prevCustomers.map(c =>
                        c._id === message.chatRoomId ? { ...c, lastMessage: message.content, lastActive: message.createdAt, unreadCount: c._id !== selectedCustomerIdRef.current ? (c.unreadCount || 0) + 1 : 0 } : c
                    );
                    return updatedCustomers.sort((a: WaitingChatRoom, b: WaitingChatRoom) => new Date(b.lastActive!).getTime() - new Date(a.lastActive!).getTime());
                }
    ...
    useEffect(() => {
        ...
        if (selectedCustomer && user) {
            const handleJoinAndFetch = async () => {
                try {
                    const isAdminMember = selectedCustomer.members.some((member: ChatRoomUser) => String(member._id) === user._id);
                    if (!isAdminMember) {
                        await joinChatRoom(selectedCustomer._id, user._id);
                        setCustomers(prev => prev.map(c => c._id === selectedCustomer._id ? { ...c, members: [...c.members, user as ChatRoomUser] } : c));
                    }
                    ...
                    const roomData = customers.find(c => c._id === selectedCustomer._id);


                    const hasUnreadMessages = roomData?.unreadCount && roomData.unreadCount > 0;
                    ...
                    if (!messages[selectedCustomer._id] || hasUnreadMessages) {
                        ...
                        setMessages(prev => ({ ...prev, [selectedCustomer._id]: messageHistory }));
                    }
    ...
    useEffect(() => { scrollToBottom(); }, [messages, selectedCustomer]);
    ...
    const handleSendReply = async () => {
        ...
        setMessages(prev => ({ ...prev, [selectedCustomer._id]: [...(prev[selectedCustomer._id] || []), optimisticMessage] }));
        setReplyMessage('');
        try {
            ...
            setMessages(prev => {
                const newRoomMessages = prev[selectedCustomer._id].map(msg => msg._id === tempId ? finalMessage : msg);
                return { ...prev, [selectedCustomer._id]: newRoomMessages };
            });
        } catch (error) {
            setMessages(prev => {
                const newRoomMessages = prev[selectedCustomer._id].filter(msg => msg._id !== tempId);
                return { ...prev, [selectedCustomer._id]: newRoomMessages };
            });
            ...
            <Layout className="h-full">
                <Sider width={350} className="bg-white !border-r !border-gray-200 overflow-hidden">
                    <div className="p-4 !border-b !border-gray-100">
                        <Title level={5} className="!mb-0">Danh sách khách hàng</Title>
                    </div>
                        ...
                        <List
                            ...
                            renderItem={(customer) => (
                                ...
                                >
                                    <List.Item.Meta
                                        avatar={
                                            <Badge count={customer.unreadCount} size="small" offset={[-5, 30]}>
                                                <Avatar
                                        ...
                                        title={
                                            <Text strong className="text-gray-800">{customer.members?.[0].username}</Text>
                                        }
                                        description={
                                            <div>
                                                <Text
                                                    className={`block text-sm ${customer.unreadCount && customer.unreadCount > 0 ? 'font-bold text-gray-900' : 'text-gray-500'}`}
                                                    ellipsis={{ tooltip: customer.lastMessage }}
                                                >
                                                    {customer.lastMessage}
                                                </Text>
                                                <Text type="secondary" className="text-xs mt-1 block">
                                                    {customer.lastActive ? format(new Date(customer.lastActive), 'p, dd/MM/yy') : 'Chưa có hoạt động'}
                                                </Text>
                                            </div>
                                        }
                                    />
                        ...
                        <>
                            <div className="p-4 !border-b !border-gray-200 bg-gray-50">
                                <div className="flex items-center space-x-3">
                                    ...
                                    />
                                    <div>
                                        <Title level={4} className="!mb-1">{selectedCustomer.members?.[0].username}</Title>
                                        <Text type="secondary">
                                            {selectedCustomer.lastActive ? `Hoạt động lần cuối: ${format(new Date(selectedCustomer.lastActive), 'p, dd/MM/yy')}` : 'Đang hoạt động'}
                                        </Text>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    {(messages[selectedCustomer._id] || []).map((msg) => {
                                        ...
                                        return (
                                            <div key={msg._id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                                                ...
                                                <div className={`max-w-xs lg:max-w-md px-4 py-3 rounded-2xl shadow-sm ${isMe
                                                    ? 'bg-blue-600 text-white rounded-br-lg'
                                                    : 'bg-white text-gray-800 !border !border-gray-200 rounded-bl-lg'
                                                    }`}>
                                                    <div className="text-sm leading-relaxed">{msg.content}</div>
                                                    <div className={`text-xs mt-2 text-right ${isMe ? 'text-blue-100' : 'text-gray-400'}`}>
